<?php
    ...
    function bidRequest($bidRequestJson,$campaigns){

        $bidRequest = json_decode($bidRequestJson, true);
        $validation = validateBidRequest($bidRequest);

        if($validation === FALSE){
            echo json_encode(["error" => "Invalid bid request"], JSON_PRETTY_PRINT);
        }

        $filterData = [
            'imp'        => $bidRequest['imp'][0],
            'bidFloor'   => isset($imp['bidfloor']) ? $imp['bidfloor'] : 0,
            'geo'        => isset($bidRequest['device']['geo']) ? $bidRequest['device']['geo'] : null,
            'deviceMake' => isset($bidRequest['device']['make']) ? $bidRequest['device']['make'] : null,
            'os'         => isset($bidRequest['device']['os']) ? $bidRequest['device']['os'] : null,
        ];

        $filteredCampaigns = filteredCampaigns($filterData,$campaigns);
        // var_dump($filteredCampaigns); die();

        $selectedCampaign = null;

        if (!empty($filteredCampaigns)) {
            $selectedCampaign = array_rand($filteredCampaigns);
        }

        $response = [];

        if ($selectedCampaign) {
            $response['id'] = $bidRequest['id'];
            $response['seatbid'] = [
                [
                    'bid' => [
                        'id' => '1',
                        ...
                        'adm' => '<a href="' . $selectedCampaign['url'] . '"><img src="' . $selectedCampaign['image_url'] . '" alt="Banner Ad" width="' . $imp['banner']['w'] . '" height="' . $imp['banner']['h'] . '"></a>',
    ...
    function validateBidRequest($bidRequest){
        if (!$bidRequest || !isset($bidRequest['imp'], $bidRequest['device'], $bidRequest['app'])) {
            return FALSE;
        }else{
            return TRUE;
        }
    ...
    function filteredCampaigns($filterData,$campaigns){

    // var_dump($filterData,$campaigns); die();
        $filteredCampaigns = array_filter($campaigns, function($campaign) use ($filterData) {
            if ($campaign['price'] < $filterData['bidFloor']) {
                return false;
            }

            if ($filterData['geo'] && isset($campaign['country']) && $filterData['geo']['country'] !== $campaign['country']) {
                return false;
            }

            if ($filterData['deviceMake'] && $campaign['device_make'] !== 'No Filter' && strtolower($campaign['device_make']) !== strtolower($filterData['deviceMake'])) {
                return false;
            }

            if ($filterData['os'] && $campaign['hs_os'] !== 'No Filter' && strpos(strtolower($campaign['hs_os']), strtolower($filterData['os'])) === false) {
                return false;
            }
            return true;
        });  
        ...
        "imp": [
            {
                ...
                "ext": {
                    ...
                    "ampad": 2,
                    "creative_enforcement_settings": {"policy_enforcement": 2, "scan_enforcement": 1, "publisher_blocks_enforcement": 1},
    ...
    echo bidRequest($bidRequestJson,$campaigns);
?>
